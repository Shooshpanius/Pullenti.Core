...
namespace Pullenti.Morph.Internal
{
    public class MorphEngine
    {
        ...
        public List<Pullenti.Morph.MorphWordForm> Process(string word, bool ignoreNoDict = false)
        {
            ...
            Pullenti.Morph.MorphWordForm toFirst = null;
            if (res != null) 
            {
                foreach (Pullenti.Morph.MorphWordForm r in res) 
                {
                    ...
                    if (res.Count > 1 && toFirst == null) 
                    {
                        if (((((r.NormalFull == "КОПИЯ" || r.NormalFull == "ПОЛК" || r.NormalFull == "СУД") || r.NormalCase == "МОРЕ" || r.NormalFull == "ПАРК") || r.NormalFull == "БАНК" || r.NormalCase == "МАРИЯ") || r.NormalCase == "ВЕТЕР" || r.NormalCase == "КИЕВ") || r.NormalFull == "ПОЛОЖЕНИЕ") 
                            toFirst = r;
                    }
                }
            }
            if (toFirst != null && res[0] != toFirst) 
            {
                res.Remove(toFirst);
                res.Insert(0, toFirst);
            }
        ...
        public void GetAllWordsByClass(Pullenti.Morph.MorphClass cla, List<Pullenti.Morph.MorphWordForm> res, StringBuilder tmp)
        {
            this._getAllWordsByClass(cla, m_Root, res, tmp);
        }
        void _getAllWordsByClass(Pullenti.Morph.MorphClass cla, MorphTreeNode tn, List<Pullenti.Morph.MorphWordForm> res, StringBuilder tmp)
        {
            if (tn.LazyPos > 0) 
                this._loadTreeNode(tn);
            if (tn.RuleIds != null) 
            {
                ...
                {
                    MorphRule r = this.GetRule(rid);
                    for (int i = 0; i < r.MorphVars.Count; i++) 
                    {
                        List<MorphRuleVariant> v = r.MorphVars[i];
                        Pullenti.Morph.MorphWordForm wf = null;
                        foreach (MorphRuleVariant vv in v) 
                        {
                            if (!((vv.Class & cla)).IsUndefined) 
                            {
                                if (!vv.Case.IsUndefined && !vv.Case.IsNominative) 
                                    continue;
                                if (((vv.Number & Pullenti.Morph.MorphNumber.Singular)) != Pullenti.Morph.MorphNumber.Singular && vv.Number != Pullenti.Morph.MorphNumber.Undefined) 
                                    continue;
                                wf = new Pullenti.Morph.MorphWordForm();
                                wf.CopyFrom(vv);
                                wf.NormalCase = tmp.ToString();
                                if (!string.IsNullOrEmpty(r.Tails[i])) 
            ...
            if (tn.Nodes != null) 
            {
                foreach (KeyValuePair<short, MorphTreeNode> kp in tn.Nodes) 
                {
                    tmp.Append((char)kp.Key);
                    this._getAllWordsByClass(cla, kp.Value, res, tmp);
                    tmp.Length -= 1;
                }
            }
        }
        ...
        int _calcEqCoef(MorphRuleVariant v, Pullenti.Morph.MorphWordForm wf)
        {
            ...
            if (wf.Misc != null && v.MiscInfoId != wf.Misc.Id) 
            {
