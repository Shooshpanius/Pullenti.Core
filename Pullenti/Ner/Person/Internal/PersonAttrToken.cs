...
namespace Pullenti.Ner.Person.Internal
{
    public class PersonAttrToken : Pullenti.Ner.ReferentToken
    {
                ...
                Pullenti.Ner.Core.NounPhraseToken npt = Pullenti.Ner.Core.NounPhraseHelper.TryParse(res.BeginToken, Pullenti.Ner.Core.NounPhraseParseAttr.No, 0, null);
                    {
                        ...
                        if (tt0 != null) 
                        {
                            ...
                            for (Pullenti.Ner.Token tt = tt0.Previous; tt != null && (cou < 20); tt = tt.Previous,cou++) 
                            {
            ...
            for (int step = 0; step < 2; step++) 
            {
                ...
                if (toks != null) 
                {
                    foreach (Pullenti.Ner.Core.TerminToken tok in toks) 
                    {
                        ...
                        {
                            if (pat.CanonicText == "ГЛАВА") 
                            {
                                Pullenti.Ner.Core.ComplexNumToken num = Pullenti.Ner.Core.ComplexNumToken.TryParse(tok.EndToken.Next, null, true, false);
                                if (num != null) 
                                    return null;
                            }
        ...
        static PersonAttrToken CreateAttrPosition(Pullenti.Ner.Core.TerminToken tok, PersonAttrAttachAttrs attrs)
        {
                    ...
                    for (; ts != null; ts = ts.Next) 
                    {
                        if (ts.Morph.Class.IsPreposition) 
                        {
                            ...
                            if (ts.IsValue("ПО", null) && ts.Next != null) 
                            {
                                ...
                                {
                                    ts = (te = nnn.EndToken);
                                    if (ts.Next != null && ts.Next.IsCommaAnd) 
                                    {
                                        for (Pullenti.Ner.Token tt2 = ts.Next.Next; tt2 != null; tt2 = tt2.Next) 
                                        {
                                            Pullenti.Ner.Core.NounPhraseToken nnn2 = Pullenti.Ner.Core.NounPhraseHelper.TryParse(tt2, Pullenti.Ner.Core.NounPhraseParseAttr.No, 0, null);
                                            if (nnn2 == null) 
                                                break;
                                            if (((nnn2.Morph.Case & nnn.Morph.Case)).IsUndefined) 
                                                break;
                                            ...
                                            tt2 = nnn2.EndToken;
                                            if (tt2.Next != null && tt2.Next.IsCommaAnd) 
                                                tt2 = tt2.Next;
                                            else 
                                                break;
                                        }
            ...
            if (name == "русский") 
            {
                Pullenti.Ner.Core.NounPhraseToken npt = Pullenti.Ner.Core.NounPhraseHelper.TryParse(tok.BeginToken, Pullenti.Ner.Core.NounPhraseParseAttr.No, 0, null);
            ...
            for (Pullenti.Ner.Token t = t1.Next; t != null; t = t.Next) 
            {
                ...
                {
                    if (t.IsValue("ПО", null) || t.IsValue("В", null)) 
                        break;
                }
                if (t == t1.Next) 
                {
                ...
                if (pat != null) 
                {
                    ...
                    else if (((!tok.Morph.Case.IsGenitive && (tok.Termin is PersonAttrTermin) && (tok.Termin as PersonAttrTermin).IsKin) && pat.Morph.Case.IsGenitive && pat.PropRef != null) && pat.PropRef.Kind == Pullenti.Ner.Person.PersonPropertyKind.Kin) 
                    {
                        t1 = (tname1 = (t = pat.EndToken));
                        continue;
                    }
                        ...
                        if (pits != null && pits.Count > 0) 
                        {
                            if (pits.Count == 2) 
                            {
                                ...
                                if (pits[1].Typ == PersonItemToken.ItemType.Initial && pits[0].Lastname != null) 
                                    break;
                                if (pits[0].Lastname != null && pits[1].Firstname != null) 
                                    break;
                            }
                            if (pits.Count == 3) 
                            {
                ...
                if (npt != null) 
                {
                    if (_existsInDoctionary(npt.EndToken) && (((((npt.Morph.Case.IsGenitive || npt.Morph.Case.IsInstrumental || npt.EndToken.IsValue("НАПРАВЛЕНИЕ", null)) || npt.EndToken.IsValue("ОТДЕЛ", null) || npt.EndToken.IsValue("ОТДЕЛЕНИЕ", null)) || npt.EndToken.IsValue("ДЕПАРТАМЕНТ", null) || npt.EndToken.IsValue("СЛУЖБА", null)) || npt.EndToken.IsValue("ПОДРАЗДЕЛЕНИЕ", null) || npt.EndToken.IsValue("УПРАВЛЕНИЕ", null)))) 
                    {
                        test = true;
                        isDepart = true;
                    }
