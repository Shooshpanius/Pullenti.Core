            ...
            if (!shortVariant) 
            {
                ...
                foreach (Pullenti.Ner.Slot r in Slots) 
                {
                    if (r.TypeName == ATTR_AUTHOR && (r.Value is Pullenti.Ner.Referent)) 
                        res.AppendFormat(", {0}", (r.Value as Pullenti.Ner.Referent).ToStringEx(true, lang, 0));
                }
            }
            if (City != null && !shortVariant) 
                res.AppendFormat(", {0}", City.ToStringEx(true, lang, 0));
            if (Date != null) 
            {
                if (!shortVariant) 
                    res.AppendFormat(", {0}", Date.ToStringEx(true, lang, 0));
                else 
