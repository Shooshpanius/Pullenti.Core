...
namespace Pullenti.Ner.Core.Internal
{
    public class BlockLine : Pullenti.Ner.MetaToken
    {
        ...
        public static BlockLine Create(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection names)
        {
            ...
            while (t != null && t.Next != null && t.EndChar <= res.EndChar) 
            {
                if (t is Pullenti.Ner.NumberToken) 
                {
                    if ((t as Pullenti.Ner.NumberToken).Typ == Pullenti.Ner.NumberSpellingType.Words) 
                    {
                        if (t.Next == null) 
                            break;
                        if ((t.Next.IsValue("ЧАСТЬ", null) || t.Next.IsValue("СТАТЬЯ", null) || t.Next.IsValue("ГЛАВА", null)) || t.Next.IsValue("РАЗДЕЛ", null)) 
                        {
                        }
                        else 
                            break;
            ...
            {
                if ((t is Pullenti.Ner.NumberToken) && (t as Pullenti.Ner.NumberToken).Typ != Pullenti.Ner.NumberSpellingType.Digit) 
                {
                    res.Words++;
