...
namespace Pullenti.Ner.Person
{
    ...
    public class PersonAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        public override void Process(Pullenti.Ner.Core.AnalysisKit kit)
        {
            ...
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                ...
                if (pers.Count == 1) 
                {
                    ...
                    {
                        Pullenti.Ner.ReferentToken rt1 = new Pullenti.Ner.ReferentToken(ppr, pat.BeginToken, pat.EndToken) { Morph = pat.Morph };
                        kit.EmbedToken(rt1);
        ...
        internal static Pullenti.Ner.ReferentToken TryAttachPerson(Pullenti.Ner.Token t, bool forExtOntos, int step, bool forAttribute = false)
        {
            ...
            if ((res == null && t.IsNewlineBefore && (t.BeginChar < 1000)) && step == 0) 
                res = _tryAttachHello(t);
            ad.OverflowLevel--;
            return res;
        }
        static Pullenti.Ner.ReferentToken _tryAttachHello(Pullenti.Ner.Token t)
        {
            Pullenti.Ner.Core.TerminToken tok = Pullenti.Ner.Mail.Internal.MailLine.m_HelloWords.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No);
            if (tok == null) 
                return null;
            for (t = tok.EndToken.Next; t != null; t = t.Next) 
            {
                if (t.LengthChar == 1) 
                    continue;
                List<Pullenti.Ner.Person.Internal.PersonItemToken> pits = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(t, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.NominativeCase, 3);
                if (pits == null || pits.Count != 2) 
                    break;
                if (pits[0].Firstname == null || pits[1].Middlename == null) 
                    break;
                int mal = 0;
                int fem = 0;
                foreach (Pullenti.Ner.Person.Internal.PersonItemToken.MorphPersonItemVariant v in pits[0].Firstname.Vars) 
                {
                    if (v.Gender == Pullenti.Morph.MorphGender.Feminie) 
                        fem++;
                    else if (v.Gender == Pullenti.Morph.MorphGender.Masculine) 
                        mal++;
                }
                foreach (Pullenti.Ner.Person.Internal.PersonItemToken.MorphPersonItemVariant v in pits[1].Middlename.Vars) 
                {
                    if (v.Gender == Pullenti.Morph.MorphGender.Feminie) 
                        fem++;
                    else if (v.Gender == Pullenti.Morph.MorphGender.Masculine) 
                ...
                for (t = tok.BeginToken.Previous; t != null; t = t.Previous) 
                {
                    PersonReferent pers0 = t.GetReferent() as PersonReferent;
                    if (pers0 == null) 
                        continue;
                    string nam = pers0.GetStringValue(PersonReferent.ATTR_FIRSTNAME);
                    if (nam == null) 
                        continue;
                    if (!pits[0].Firstname.Term.StartsWith(nam)) 
                        continue;
                    string sec = pers0.GetStringValue(PersonReferent.ATTR_MIDDLENAME);
                    if (sec == null) 
                        continue;
                    if (!pits[1].Middlename.Term.StartsWith(sec)) 
                        continue;
                    PersonReferent pers = new PersonReferent();
                    pers.AddSlot(PersonReferent.ATTR_FIRSTNAME, pits[0].Firstname.Term, true, 0);
                    pers.AddSlot(PersonReferent.ATTR_MIDDLENAME, pits[1].Middlename.Term, true, 0);
                    foreach (string fam in pers0.GetStringValues(PersonReferent.ATTR_LASTNAME)) 
                    {
                        pers.AddSlot(PersonReferent.ATTR_LASTNAME, fam, false, 0);
                    }
                    if (mal > fem) 
                        pers.IsMale = true;
                    else if (mal < fem) 
                        pers.IsFemale = true;
                    return new Pullenti.Ner.ReferentToken(pers, pits[0].BeginToken, pits[1].EndToken);
                }
                int cou = 100;
                for (t = tok.BeginToken.Previous; t != null && cou > 0; t = t.Previous,cou--) 
                {
                    if (t.Chars.IsAllLower) 
                        continue;
                    if (!t.IsNewlineBefore) 
                        continue;
                    List<Pullenti.Ner.Person.Internal.PersonItemToken> pits2 = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(t, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.No, 4);
                    if (pits2 == null || pits2.Count != 3) 
                        continue;
                    if (!pits2[2].IsNewlineAfter) 
                        continue;
                    if (pits2[0].Typ != Pullenti.Ner.Person.Internal.PersonItemToken.ItemType.Value || pits2[1].Typ != Pullenti.Ner.Person.Internal.PersonItemToken.ItemType.Initial || pits2[2].Typ != Pullenti.Ner.Person.Internal.PersonItemToken.ItemType.Initial) 
                        continue;
                    if (!pits[0].Value.StartsWith(pits2[1].Value) || !pits[1].Value.StartsWith(pits2[2].Value)) 
                        continue;
                    PersonReferent pers = new PersonReferent();
                    pers.AddSlot(PersonReferent.ATTR_FIRSTNAME, pits[0].Firstname.Term, true, 0);
                    pers.AddSlot(PersonReferent.ATTR_MIDDLENAME, pits[1].Middlename.Term, true, 0);
                    if (mal > fem) 
                        ...
                        pers.IsFemale = true;
                    if (pits2[0].Lastname != null) 
                    {
                        foreach (Pullenti.Ner.Person.Internal.PersonItemToken.MorphPersonItemVariant v in pits2[0].Lastname.Vars) 
                        {
                            if (v.Case.IsInstrumental) 
                            {
                                pers.AddSlot(PersonReferent.ATTR_LASTNAME, v.Value, true, 0);
                                break;
                            }
                        }
                        if (pers.FindSlot(PersonReferent.ATTR_LASTNAME, null, true) == null) 
                        {
                            foreach (Pullenti.Ner.Person.Internal.PersonItemToken.MorphPersonItemVariant v in pits2[0].Lastname.Vars) 
                            {
                                if (((mal > fem && v.Gender == Pullenti.Morph.MorphGender.Masculine)) || (((mal < fem) && v.Gender == Pullenti.Morph.MorphGender.Feminie))) 
                                {
                                    pers.AddSlot(PersonReferent.ATTR_LASTNAME, v.Value, true, 0);
                                    break;
                                }
                    ...
                    if (pers.FindSlot(PersonReferent.ATTR_LASTNAME, null, true) == null) 
                        pers.AddSlot(PersonReferent.ATTR_LASTNAME, pits2[0].Value, true, 0);
                    return new Pullenti.Ner.ReferentToken(pers, pits[0].BeginToken, pits[1].EndToken);
                }
                break;
            }
        ...
        static Pullenti.Ner.ReferentToken _tryAttachPersonInt(Pullenti.Ner.Token t, bool forExtOntos, int step, bool forAttribute = false)
        {
            ...
            if (attrs != null) 
            {
                ...
                while (t != null) 
                {
                    if (t.IsTableControlChar) 
                        t = t.Next;
                    else if (t.IsChar('[') && t.Next != null && ((t.Next.IsValue3("МЕСТО", "ДЛЯ", "ПЕЧАТИ") || t.Next.IsValue3("МЕСТО", "ДЛЯ", "ШТАМПА")))) 
                    {
            ...
            for (int k = 0; k < 2; k++) 
            {
                    ...
                    if ((attrs != null && attrs.Count > 0 && attrs[attrs.Count - 1].BeginToken == attrs[attrs.Count - 1].EndToken) && attrs[attrs.Count - 1].BeginToken.Chars.IsCapitalUpper) 
                    {
                        ...
                        else 
                        {
                            ...
                            if (pits1 != null && pits1[0].Lastname != null) 
                            {
                                ...
                                {
                                    if (pits1.Count == 3 && pits1[1].BeginToken.IsValue("РОМАН", null) && pits1[2].BeginToken.IsValue("АБРАМОВИЧ", null)) 
                                    {
                                    }
                                    else 
                                    {
                ...
                if (pli != null && pli.Count > 0) 
                {
                    ...
                    if (best.Coef >= 1 && (best.Coef < minCoef) && attrs == null) 
                    {
                        for (Pullenti.Ner.Token tt = t.Previous; tt != null; tt = tt.Previous) 
                        {
                            ...
                            {
                                if (tt.IsValue("ИСПОЛНИТЕЛЬ", null) || tt.IsValue("ИСП", null)) 
                                    best.Coef += 1;
                                break;
                            }
                        }
