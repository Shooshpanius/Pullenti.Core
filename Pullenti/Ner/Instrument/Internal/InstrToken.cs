    ...
    class InstrToken : Pullenti.Ner.MetaToken
    {
        ...
        public static List<InstrToken> ParseList(Pullenti.Ner.Token t0, int maxChar = 0)
        {
            ...
            for (Pullenti.Ner.Token t = t0; t != null; t = t.Next) 
            {
                ...
                else if (t.IsValue("ПРИЛОЖЕНИЕ", null)) 
                {
                }
                ...
                if (it.Typ == ILTypes.Undefined && t.IsNewlineBefore && !t.IsTableControlChar) 
                {
        ...
        public static InstrToken Parse(Pullenti.Ner.Token t, int maxChar = 0, InstrToken prev = null)
        {
            ...
            if (t != null) 
            {
                ...
                {
                    if (t.Next != null && t.Next.IsValue("ПРИЛОЖЕНИЕ", null)) 
                    {
                    }
                    else 
                    {
                        List<Pullenti.Ner.Core.TableRowToken> rows = Pullenti.Ner.Core.TableHelper.TryParseRows(t, maxChar, true, false);
                        if (rows != null && rows.Count > 3) 
                            return new InstrToken(t, rows[rows.Count - 1].EndToken) { Typ = ILTypes.Undefined };
                    }
                }
                isStartOfLine = t00.IsNewlineBefore;
                if (t.Previous != null && t.Previous.IsChar((char)0x1F)) 
                    isStartOfLine = true;
                while (t != null) 
                {
            ...
            for (; t != null; t = t.Next) 
            {
                ...
                if (isStartOfLine && t == t0) 
                {
                    ...
                    if (t.IsValue("ИСП", null)) 
                    {
                        Pullenti.Ner.Token tt2 = t.Next;
                        if (tt2 != null && tt2.IsChar('.')) 
                            tt2 = tt2.Next;
                        InstrToken next = Parse(tt2, 0, null);
                        if (next != null && next.Typ == ILTypes.Person) 
                        {
                            next.BeginToken = t;
                            return next;
                    ...
                    if (npt != null && npt.Morph.Case.IsNominative && (npt.EndToken is Pullenti.Ner.TextToken)) 
                    {
                        ...
                        if (((term1 == "ПРИЛОЖЕНИЕ" || term1 == "ДОДАТОК" || term1 == "МНЕНИЕ") || term1 == "ДУМКА" || term1 == "АКТ") || term == "ЗАЯВКА") 
                        {
                            ...
                            if (tt1 == null) 
                                ...
                                ok = false;
                            else if (term1 == "МНЕНИЕ" || term1 == "ДУМКА") 
                            {
                                ok = false;
                                if (tt1 != null) 
                                {
                                    if (tt1.IsValue("СУДЬЯ", null)) 
                                        ok = true;
                                    else if ((tt1 is Pullenti.Ner.ReferentToken) && (tt1 as Pullenti.Ner.ReferentToken).Referent.TypeName == "PERSON") 
                                        ok = true;
                                }
                            }
                            if (ok) 
        ...
        internal static List<string> m_Directives = new List<string>(new string[] {"ПРИКАЗЫВАТЬ", "ПРИКАЗАТЬ", "ПРЕДЛАГАТЬ", "ОПРЕДЕЛЯТЬ", "ОПРЕДЕЛЯТЬ", "ОПРЕДЕЛИТЬ", "ПОСТАНОВЛЯТЬ", "ПОСТАНОВИТЬ", "УСТАНОВИТЬ", "РЕШИЛ", "РЕШИТЬ", "ПРОСИТЬ", "ПРИГОВАРИВАТЬ", "ПРИГОВОРИТЬ", "НАКАЗУВАТИ", "ВИЗНАЧАТИ", "ВИЗНАЧИТИ", "УХВАЛЮВАТИ", "УХВАЛИТИ", "ПОСТАНОВЛЯТИ", "ПОСТАНОВИТИ", "ВСТАНОВИТИ", "ВИРІШИВ", "ВИРІШИТИ", "ПРОСИТИ", "ПРИМОВЛЯТИ", "ЗАСУДИТИ"});
        internal static List<string> m_DirectivesNorm = new List<string>(new string[] {"ПРИКАЗ", "ПРИКАЗ", "ПРИКАЗ", "ОПРЕДЕЛЕНИЕ", "ОПРЕДЕЛЕНИЕ", "ОПРЕДЕЛЕНИЕ", "ПОСТАНОВЛЕНИЕ", "ПОСТАНОВЛЕНИЕ", "УСТАНОВЛЕНИЕ", "РЕШЕНИЕ", "РЕШЕНИЕ", "ЗАЯВЛЕНИЕ", "ПРИГОВОР", "ПРИГОВОР", "НАКАЗ", "УХВАЛА", "УХВАЛА", "УХВАЛА", "УХВАЛА", "ПОСТАНОВА", "ПОСТАНОВА", "ВСТАНОВЛЕННЯ", "РІШЕННЯ", "РІШЕННЯ", "ЗАЯВА", "ВИРОК", "ВИРОК"});
    }
}