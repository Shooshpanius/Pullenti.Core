...
namespace Pullenti.Ner
{
    ...
    public class Token
    {
        ...
        public override string ToString()
        {
            int end = EndChar;
            bool err = false;
            if (end >= Kit.Sofa.Text.Length) 
            {
                end = Kit.Sofa.Text.Length - 1;
                err = true;
            }
            if (BeginChar > end) 
                return "?";
        ...
        bool GetAttr(int i)
        {
            ...
            if (((m_Attrs & 1)) == 0) 
            {
                ...
                if (m_Next == null) 
                {
                    ...
                    for (int j = EndChar + 1; (j < m_Next.BeginChar) && (j < Kit.Sofa.Text.Length); j++) 
                    {
                        if (char.IsWhiteSpace((ch = Kit.Sofa.Text[j]))) 
                        {
                            this.SetAttr(2, true);
        ...
        public int NewlinesBeforeCount
        {
            get
            {
                ...
                for (int p = BeginChar - 1; p >= 0 && (p < txt.Length); p--) 
                {
                    char ch = txt[p];
                    if (ch == 0xA || ch == 0x2028) 
                        res++;
        ...
        public virtual bool IsCharOf(string chars)
        {
            ...
            if (BeginChar >= Kit.Sofa.Text.Length) 
                return false;
            if (chars.IndexOf(Kit.Sofa.Text[BeginChar]) >= 0) 
                return true;
            if (chars.IndexOf('-') >= 0) 
            {
                if (IsHiphen) 
