...
namespace Pullenti.Semantic
{
    ...
    public class SemBlock : ISemContainer
    {
        ...
        public void AddFragmentVariantas(SemBlock blk)
        {
            foreach (SemFragment fr in blk.Fragments) 
            {
                foreach (SemFragment fr1 in Fragments) 
                {
                    if (fr1.BeginChar == fr.BeginChar && fr1.EndChar == fr.EndChar) 
                    {
                        fr1.AltFragments.Add(fr);
                        fr.m_Higher = this;
                        break;
                    }
                }
