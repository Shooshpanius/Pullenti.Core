...
namespace Pullenti.Ner.Instrument
{
    ...
    public class InstrumentParticipantReferent : Pullenti.Ner.Referent
    {
        ...
        public override string ToStringEx(bool shortVariant, Pullenti.Morph.MorphLang lang, int lev = 0)
        {
            ...
            if (org != null) 
            {
                res.AppendFormat(": {0}", org.ToStringEx(shortVariant, lang, 0));
                if (!shortVariant && del != null) 
                    res.AppendFormat(" (в лице {0})", del.ToStringEx(true, lang, lev + 1));
            }
            else if (del != null) 
                res.AppendFormat(": в лице {0}", del.ToStringEx(shortVariant, lang, lev + 1));
            return res.ToString();
        }
