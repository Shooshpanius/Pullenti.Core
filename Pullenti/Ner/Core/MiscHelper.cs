    ...
    public static class MiscHelper
    {
            ...
            try 
            {
                ...
                while (t1 != null || t2 != null) 
                {
                    ...
                    if (!t1.Chars.IsLetter) 
                    {
                        ...
                        {
                            if (t1.IsValue("N", null) && t2.IsValue("№", null)) 
                            {
                            }
                            else if (t2.IsValue("N", null) && t1.IsValue("№", null)) 
                            {
                            }
                            else 
                                return false;
                    ...
                    if (((attrs & CanBeEqualsAttr.IgnoreNonletters)) != CanBeEqualsAttr.No) 
                    {
                        ...
                        if (((mc1.IsConjunction || mc1.IsPreposition)) && t1.Next != null) 
                        {
                            if (t1.Next.LengthChar == t2.LengthChar) 
                            {
                                if (t1.Next.ToString() == t2.ToString()) 
                                {
                                    t1 = t1.Next;
                                    continue;
                                }
                        ...
                        Pullenti.Morph.MorphClass mc2 = t2.GetMorphClassInDictionary();
                        if (((mc2.IsConjunction || mc2.IsPreposition)) && t2.Next != null) 
                        {
                            if (t2.Next.LengthChar == t1.LengthChar) 
                            {
                                if (t2.Next.ToString() == t1.ToString()) 
                                {
                                    t2 = t2.Next;
                                    continue;
                                }
        ...
        public static bool CanBeStartOfSentence(Pullenti.Ner.Token t)
        {
            ...
            if (t.Chars.IsLetter && t.Chars.IsAllLower) 
            {
                ...
                if (t.Previous.IsHiphen && t.Previous.IsNewlineBefore) 
                    return false;
            }
            ...
            if (t.Previous.IsChar(':')) 
                return false;
            if (t.Previous.IsCharOf(";") && t.IsNewlineBefore) 
                return true;
            ...
            if (t.Previous.IsChar('.') || (((t.Previous is Pullenti.Ner.ReferentToken) && (t.Previous as Pullenti.Ner.ReferentToken).EndToken.IsChar('.')))) 
            {
                ...
                if (t.Previous.IsChar('.') && (t.Previous.Previous is Pullenti.Ner.TextToken) && (t.WhitespacesBeforeCount < 2)) 
                {
                    if (t.Previous.Previous.IsValue("Г", null)) 
                        return false;
                }
                return true;
            }
        ...
        public static string ConvertFirstCharUpperAndOtherLower(string str)
        {
            ...
            for (i = 0; i < tmp.Length; i++) 
            {
                if (char.IsLetter(tmp[i])) 
                {
                    if (up) 
                    {
                        if (((i + 1) >= tmp.Length || char.IsLetter(tmp[i + 1]) || ((tmp[i + 1] == '.' || tmp[i + 1] == '-'))) || i == 0) 
                            tmp[i] = char.ToUpper(tmp[i]);
                    }
                    up = false;
                }
                else if (!char.IsDigit(tmp[i])) 
                    up = true;
            }
            tmp.Replace(" - ", "-");
            for (i = 1; i < (tmp.Length - 5); i++) 
            {
                if (tmp[i] == '-' && tmp[i + 3] == '-') 
                {
                    tmp[i + 1] = char.ToLower(tmp[i + 1]);
                    tmp[i + 2] = char.ToLower(tmp[i + 2]);
                    break;
                }
            }
            return tmp.ToString();
        ...
        /// </summary>
        /// <param name="name">исходный термин</param>
        /// <param name="abbr">возможная аббревиатура</param>
        /// <return>может ли быть</return>
        public static bool CheckAbbreviation(string name, string abbr)
        {
            if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(abbr)) 
                return false;
            if (name.Length <= abbr.Length) 
                return false;
            if (name.IndexOf('-') > 0) 
                name = name.Replace('-', ' ');
            ...
            if (words.Length == abbr.Length) 
            {
                ...
                if (words.Length == 2) 
                {
                    if (words[0][0] == abbr[1] && words[1][0] == abbr[0]) 
                        return true;
                }
                return false;
            }
            if (words.Length == (abbr.Length + 1)) 
            {
            ...
            if ((words.Length + 1) == abbr.Length) 
            {
                if (words[0][0] != abbr[0]) 
                    return false;
                i = words[0].IndexOf(abbr[1]);
                if ((i < 3) || (i + 3) >= words[0].Length) 
                    return false;
                for (i = 2; i < abbr.Length; i++) 
                {
                    if (string.IsNullOrEmpty(words[i - 1])) 
                        break;
                    else if (words[i - 1][0] != abbr[i]) 
                        break;
                }
        ...
        /// </summary>
        /// <return>при успехе вернёт указатель на последний токен ']'</return>
        public static Pullenti.Ner.Token CheckImage(Pullenti.Ner.Token t)
        {
            if (t == null || !t.IsChar('[') || t.Next == null) 
                return null;
            if (!t.Next.IsValue("КАРТИНКА", null)) 
                return null;
            int cou = 4;
            for (t = t.Next; t != null && cou > 0; cou--,t = t.Next) 
            {
                if (t.IsChar(']')) 
                    return t;
            }
        ...
        public static List<string> ConvertLatinWordToRussianVariants(string str)
        {
            ...
            _ConvertCyrLatWord(str, true, res, null);
            return res;
        }
        /// <summary>
        ...
        {
            List<string> res = new List<string>();
            _ConvertCyrLatWord(str, false, res, null);
            return res;
        }
        static bool _ConvertCyrLatWord(string str, bool latinToRus, List<string> res, List<string> comp)
        {
            if (string.IsNullOrEmpty(str)) 
                return false;
            str = str.ToUpper();
            ...
            while (true) 
            {
                ...
                string www = strTmp.ToString();
                if (res != null) 
                    res.Add(www);
                else if (comp.Contains(www)) 
                    return true;
        ...
        /// </summary>
        /// <param name="t">токен на одном языке</param>
        /// <param name="str">слово на другом языке</param>
        /// <return>да-нет</return>
        ...
        static string _getTextValue_(Pullenti.Ner.Token begin, Pullenti.Ner.Token end, GetTextAttr attrs, Pullenti.Ner.Referent r)
        {
            ...
            if (((attrs & ((GetTextAttr.FirstNounGroupToNominative | GetTextAttr.FirstNounGroupToNominativeSingle)))) != GetTextAttr.No) 
            {
                ...
                {
                    if (begin.IsValue("ДОЛЖНЫЙ", null)) 
                    {
                        res.Append(corrChars("ДОЛЖЕН", begin.Chars, keepChars, begin as Pullenti.Ner.TextToken));
                        begin = begin.Next;
                    }
                }
                if (((attrs & GetTextAttr.FirstNounGroupToNominative)) == GetTextAttr.FirstNounGroupToNominative) 
            ...
            for (Pullenti.Ner.Token t = begin; t != null && t.EndChar <= end.EndChar; t = t.Next) 
            {
                ...
                if (t.IsWhitespaceBefore && res.Length > 0) 
                {
                    ...
                    if (((last != ' ' && !t.IsHiphen && last != '-') && !BracketHelper.CanBeStartOfSequence(t.Previous, false, false) && !t.IsCharOf(",.;!?")) && !BracketHelper.CanBeEndOfSequence(t, false, null, false)) 
                        res.Append(' ');
                }
        ...
        /// </summary>
        /// <param name="beginToken">начальный токен</param>
        /// <param name="endToken">конечный токен (если null, то до конца текста)</param>
        public static bool CheckPureVerb(Pullenti.Ner.Token beginToken, Pullenti.Ner.Token endToken)
        {
            if (beginToken == null) 
                return false;
            for (Pullenti.Ner.Token t = beginToken; t != null; t = t.Next) 
            {
                if (endToken != null && t.BeginChar > endToken.EndChar) 
                    break;
                Pullenti.Ner.TextToken tt = t as Pullenti.Ner.TextToken;
                if (tt == null) 
                    continue;
