    ...
    static class ListHelper
    {
        ...
        class LineToken : Pullenti.Ner.MetaToken
        {
            ...
            public void CorrectBeginToken()
            {
                if (!IsListItem) 
                    return;
                if (BeginToken.IsHiphen && BeginToken.Next != null && (BeginToken.Next.EndChar < EndChar)) 
                    BeginToken = BeginToken.Next;
                else if ((Number > 0 && BeginToken.Next != null && BeginToken.Next.IsChar(')')) && BeginToken.Next.Next != null && (BeginToken.Next.Next.EndChar < EndChar)) 
        ...
        public static void CorrectAppList(List<InstrToken1> lines)
        {
            ...
            if (emptyApps > 1 && emptyApps == (apps - 1)) 
            {
                for (int i = 0; i < lines.Count; i++) 
                {
                    InstrToken1 line = lines[i];
                    ...
                    line.Numbers.Clear();
                    line.NumTyp = NumberTypes.Undefined;
                }
            }
        }
