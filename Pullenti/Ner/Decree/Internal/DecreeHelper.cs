...
namespace Pullenti.Ner.Decree.Internal
{
    ...
    public static class DecreeHelper
    {
        ...
        public static CanonicDecreeRefUri TryCreateCanonicDecreeRefUri(Pullenti.Ner.Token t)
        {
            ...
            if (dr != null) 
            {
                ...
                for (Pullenti.Ner.Token tt = (t as Pullenti.Ner.MetaToken).BeginToken; tt != null && tt.EndChar <= t.EndChar; tt = tt.Next) 
                {
                    ...
                    if (wasTyp) 
                    {
                        DecreeToken na = DecreeToken.TryAttachName(tt, dr.Typ0, true, false, false);
        ...
        public static int RemovePsevdoDecreeparts(Pullenti.Ner.Core.AnalysisKit kit)
        {
            if (kit.MiscData.ContainsKey("RemovePsevdoDecreeparts")) 
                return (int)kit.MiscData["RemovePsevdoDecreeparts"];
            int cou = 0;
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                if (t.IsNewlineBefore && t.IsNewlineAfter) 
                {
                    if (t.GetReferent() is Pullenti.Ner.Decree.DecreePartReferent) 
                    {
                        ...
                        if (tt == null) 
                            continue;
                        if (tt.IsValue("ВВЕСТИ", null) || tt.IsCommaAnd) 
                            continue;
                        t = kit.DebedToken(t);
                        cou++;
                        continue;
                ...
                if (t.IsNewlineBefore && (t.GetReferent() is Pullenti.Ner.Decree.DecreeChangeReferent)) 
                {
                    Pullenti.Ner.Decree.DecreeChangeReferent dch = t.GetReferent() as Pullenti.Ner.Decree.DecreeChangeReferent;
                    if (dch.Kind == Pullenti.Ner.Decree.DecreeChangeKind.Container) 
                    {
                        if (((t as Pullenti.Ner.ReferentToken).BeginToken.IsValue("ИЗМЕНЕНИЕ", null) || (t as Pullenti.Ner.ReferentToken).BeginToken.IsValue("ПЕРЕЧЕНЬ", null) || (t as Pullenti.Ner.ReferentToken).BeginToken.IsValue("О", null)) || (t as Pullenti.Ner.ReferentToken).BeginToken.IsValue("В", null) || ((t as Pullenti.Ner.ReferentToken).BeginToken is Pullenti.Ner.ReferentToken)) 
                        {
            ...
            if ((kit.FirstToken is Pullenti.Ner.ReferentToken) && kit.FirstToken.GetReferent().TypeName == "ORGANIZATION" && kit.FirstToken.IsNewlineAfter) 
            {
                if ((kit.FirstToken.Next is Pullenti.Ner.ReferentToken) && (kit.FirstToken.Next.GetReferent() is Pullenti.Ner.Decree.DecreeReferent)) 
                {
                    kit.DebedToken(kit.FirstToken.Next);
                    cou++;
                }
            }
            kit.MiscData.Add("RemovePsevdoDecreeparts", cou);
            return cou;
        }
    }
}