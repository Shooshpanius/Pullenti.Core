...
namespace Pullenti.Ner.Geo.Internal
{
    class OrgTypToken : Pullenti.Ner.MetaToken
    {
        ...
        static OrgTypToken _TryParse(Pullenti.Ner.Token t, bool afterTerr, int lev = 0)
        {
            ...
            if (t is Pullenti.Ner.TextToken) 
            {
                ...
                if (term == "СТ") 
                {
                    ...
                    if (cit != null && cit.Typ == CityItemToken.ItemType.Noun && cit.LengthChar > 3) 
                        return null;
                    for (Pullenti.Ner.Token tt = t.Previous; tt != null && (tt.EndChar + 10) > t.BeginChar; tt = tt.Previous) 
                    {
                        cit = CityItemToken.TryParse(tt, null, false, null);
                        if (cit != null && cit.Typ == CityItemToken.ItemType.Noun && cit.EndChar > t.BeginChar) 
                            return null;
                    }
                }
                if (term == "БО" && t.Next != null && t.Next.IsValue("ЛЕТ", null)) 
                    return null;
                if (term == "ГО" && t.Previous != null) 
                {
                    if (t.Previous.IsHiphen || (t.Previous is Pullenti.Ner.NumberToken)) 
                        return null;
                }
                if (term == "ПГТ") 
                    return null;
                ...
                if (((((t.IsValue("ОФИС", null) || term == "ФАД" || term == "АД") || term == "МИРА" || term == "МАЯ") || t.IsValue("КОРПУС", null) || t.IsValue("ПОЛК", null)) || t.IsValue("ДИВИЗИЯ", null) || t.IsValue("ГВАРДИЯ", null)) || t.IsValue("АРМИЯ", null) || t.IsValue("РОТА", null)) 
                    return null;
                if ((t.IsValue("ФЕДЕРАЦИЯ", null) || t.IsValue("СОЮЗ", null) || t.IsValue("ПРЕФЕКТУРА", null)) || t.IsValue("ОТДЕЛЕНИЕ", null)) 
                    return null;
                if (t.IsValue("РАДИО", null) || t.IsValue("АППАРАТ", null) || t.IsValue("ДВИЖЕНИЕ", null)) 
                    return null;
                ...
                if (t.IsValue("РЕКА", null) && t.Previous != null) 
                {
                    if (t.Previous.IsValue("НАБЕРЕЖНАЯ", null) || t.Previous.IsValue("НАБ", null) || ((t.Previous.Previous != null && t.Previous.Previous.IsValue("НАБ", null)))) 
                        return null;
                }
                if (term == "АО") 
                {
                ...
                if ((((term == "СХ" || term == "CX")) && t.Next != null && t.Next.IsHiphen) && t.Next.Next != null) 
                {
                    if (t.Next.Next.IsValue("З", null) || (((t.Next.Next is Pullenti.Ner.NumberToken) && (t.Next.Next as Pullenti.Ner.NumberToken).Value == "3"))) 
                        return new OrgTypToken(t, t.Next.Next, "совхоз");
                }
            ...
            if (tok != null) 
            {
                if (tok.EndToken.IsValue("УЧАСТОК", null)) 
                {
                    for (Pullenti.Ner.Token tt = t.Next; tt != null && (tt.EndChar < tok.EndChar); tt = tt.Next) 
                    {
                        if (tt.IsComma) 
                            return null;
                    }
                ...
                if (val == "аэропорт") 
                {
                    if (t.IsValue("А", null) && tok.EndToken.IsValue("П", null)) 
                    {
                        if (NumToken.TryParse(tok.EndToken.Next, GeoTokenType.Any) != null) 
                            return new OrgTypToken(t, tok.EndToken, "аптечный пункт") { IsBuilding = true };
                    }
                }
                ...
                for (Pullenti.Ner.Token tt = tok.EndToken.Next; tt != null; tt = tt.Next) 
                {
                    if (!(tt is Pullenti.Ner.TextToken) || tt.WhitespacesBeforeCount > 2) 
                        break;
                    if ((tt.IsChar('(') && (tt.Next is Pullenti.Ner.TextToken) && (tt.Next.LengthChar < 3)) && tt.Next.Next != null && tt.Next.Next.IsChar(')')) 
                    {
                        tt = tt.Next.Next;
                        tok.EndToken = tt;
                        continue;
                ...
                if ((tok.Termin.Tag is Pullenti.Ner.Address.Internal.StreetItemType) && ((Pullenti.Ner.Address.Internal.StreetItemType)tok.Termin.Tag) == Pullenti.Ner.Address.Internal.StreetItemType.StdName) 
                    canbesingle = true;
                if (tok.Termin.Tag is Pullenti.Ner.Address.Internal.AddressItemType) 
                    building = ((Pullenti.Ner.Address.Internal.AddressItemType)tok.Termin.Tag) == Pullenti.Ner.Address.Internal.AddressItemType.Building;
                typs.Add(val);
                ...
                if (tok.EndToken == t) 
                {
                    if ((t.LengthChar < 4) && (t is Pullenti.Ner.TextToken) && Pullenti.Morph.LanguageHelper.EndsWith((t as Pullenti.Ner.TextToken).Term, "К")) 
                    {
                        ...
                        if (oi != null) 
                        {
                            ...
                            else if (MiscLocationHelper.IsUserParamAddress(t)) 
                            {
                            }
                            else 
                                return null;
            ...
            else 
            {
                ...
                if (rtok != null) 
                {
                    ...
                    if (rtok.BeginToken != rtok.EndToken) 
                    {
                        ...
                        if (rtok.Referent.ToString().ToUpper() == "ЖЕЛЕЗНАЯ ДОРОГА") 
                        {
                            if (rtok.EndToken.Next is Pullenti.Ner.TextToken) 
                            {
                                if ((rtok.EndToken.Next as Pullenti.Ner.TextToken).Term.StartsWith("СТ")) 
                                    return null;
                            }
                        }
                    }
                    ...
                    if (typs.Count > 0) 
                    {
                        if (typs[0].EndsWith("центр") || typs[0].EndsWith("комплекс") || typs[0] == "бизнес квартал") 
                            building = true;
                    }
                }
            }
            if (((t1 == null && (t is Pullenti.Ner.TextToken) && t.LengthChar >= 2) && t.LengthChar <= 4 && t.Chars.IsAllUpper) && t.Chars.IsCyrillicLetter) 
            {
                ...
                if (cit != null) 
                {
                    if (cit.Typ == CityItemToken.ItemType.Noun || cit.Typ == CityItemToken.ItemType.City) 
                        return null;
                }
                typs = new List<string>();
            ...
            if (morph == null) 
                morph = t1.Morph;
            OrgTypToken res = new OrgTypToken(t, t1) { IsDoubt = doubt, Vals = typs, Morph = morph, NotOrg = notorg, NotGeo = notgeo, CanBeSingle = canbesingle, IsBuilding = building };
        ...
        public static void Initialize()
        {
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПОТРЕБИТЕЛЬСКИЙ САДОВОДЧЕСКИЙ КООПЕРАТИВ");
            m_Ontology.Add(t);
            ...
            t.AddVariant("СПКСТ", false);
            t.AddVariant("СПК СТ", false);
            t.AddAbridge("СПК(СТ)");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("САДОВО ОГОРОДНОЕ НЕКОММЕРЧЕСКОЕ ОБЪЕДИНЕНИЕ") { Acronym = "СОНО", AcronymCanBeSmart = true, AcronymCanBeLower = true };
            t.AddVariant("САДОВО ОГОРОДНОЕ НЕКОММЕРЧЕСКОЕ ОБЪЕДИНЕНИЕ ГРАЖДАН", false);
            m_Ontology.Add(t);
            ...
            t = new Pullenti.Ner.Core.Termin("БЛОК ГАРАЖЕЙ");
            ...
            t.AddVariant("ПОДХОЗ", false);
            t.AddAbridge("ПОД.ХОЗ.");
            t.AddAbridge("П/Х");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОМПЛЕКС ПРОИЗВОДСТВЕННЫХ ЗДАНИЙ") { Tag3 = 1 };
            t.AddVariant("КОМПЛЕКС ПРОИЗВОДСТВЕННЫХ СТРОЕНИЙ", false);
            t.AddVariant("КОМПЛЕКС ПРОИЗВОДСТВЕННЫХ ЗДАНИЙ И СООРУЖЕНИЙ", false);
            t.AddVariant("КОМПЛЕКС ПРОИЗВОДСТВЕННЫХ СТРОЕНИЙ И СООРУЖЕНИЙ", false);
            m_Ontology.Add(t);
            ...
            t.AcronymCanBeLower = true;
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ГАРАЖНОЕ ОБЩЕСТВО") { Acronym = "ГО", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПЛОЩАДКА ГАРАЖЕЙ") { Tag3 = 1 };
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(new Pullenti.Ner.Core.Termin("ГАРАЖНО САРАЙНО СТРОИТЕЛЬНЫЙ КООПЕРАТИВ") { Acronym = "ГССК", AcronymCanBeLower = true, AcronymCanBeSmart = true });
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ГАРАЖНЫЙ ЭКСПЛУАТАЦИОННО-СТРОИТЕЛЬНЫЙ ПОТРЕБИТЕЛЬСКИЙ КООПЕРАТИВ") { Acronym = "ГЭСПК", AcronymCanBeLower = true, AcronymCanBeSmart = true };
            m_Ontology.Add(t);
            ...
            t.AddVariant("КССГ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОЛЛЕКТИВНАЯ СТОЯНКА ГАРАЖЕЙ");
            t.AddVariant("КОЛЛЕКТИВНАЯ ГАРАЖНАЯ СТОЯНКА", false);
            t.AddVariant("КОЛЛЕКТИВНЫЙ ГАРАЖ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОЛЛЕКТИВНАЯ ЗАСТРОЙКА ИНДИВИДУАЛЬНЫХ ГАРАЖЕЙ");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(new Pullenti.Ner.Core.Termin("ПОТРЕБИТЕЛЬСКОЕ ГАРАЖНОЕ ОБЩЕСТВО") { Acronym = "ПГО", AcronymCanBeLower = true, AcronymCanBeSmart = true });
            ...
            m_Ontology.Add(new Pullenti.Ner.Core.Termin("ИНДИВИДУАЛЬНЫЙ ГАРАЖНО СТРОИТЕЛЬНЫЙ КООПЕРАТИВ") { Acronym = "ИГСК", AcronymCanBeLower = true, AcronymCanBeSmart = true });
            ...
            t.AddAbridge("САН.");
            t.AddVariant("САНАТОРИЙ-ПРОФИЛАКТОРИЙ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПАНСИОНАТ");
            t.AddAbridge("ПАНС.");
            t.AddVariant("ТУРИСТИЧЕСКИЙ ПАНСИОНАТ", false);
            t.AddAbridge("ТУР.ПАНСИОНАТ");
            t.AddVariant("ПАНСИОНАТ С ЛЕЧЕНИЕМ", false);
            t.AddVariant("ПАНСИОНАТ ОТДЫХА", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПРОФИЛАКТОРИЙ");
            m_Ontology.Add(t);
            ...
            t.AddAbridge("Д/О");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("МЕСТО ОТДЫХА");
            m_Ontology.Add(t);
            ...
            t.AddVariant("БАЗА ОХОТХОЗЯЙСТВА", false);
            t.AddVariant("БАЗА ОХОТОХОЗЯЙСТВА", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТУРИСТИЧЕСКАЯ БАЗА") { Acronym = "ТБ", AcronymCanBeLower = true };
            t.AddAbridge("Т/Б");
            t.AddVariant("ТУРБАЗА", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПАКР ОТЕЛЬ");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("МОЛОЧНАЯ ФЕРМА");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СЕМЕННОЙ ЦЕНТР");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПИТОМНИК");
            t.AddVariant("ПИТОМНИК РАСТЕНИЙ", false);
            t.AddVariant("ПИТОМНИК ДЕКОРАТИВНЫХ РАСТЕНИЙ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПЛЕМЕННОЙ ПИТОМНИК");
            t.AddVariant("ПЛЕМПИТОМНИК", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПЛЕМЕННОЙ ЗАВОД");
            t.AddVariant("ПЛЕМЗАВОД", false);
            t.AddAbridge("ПЛЕМ.ЗАВОД");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СЕЛЬХОЗАРТЕЛЬ");
            t.AddVariant("СЕЛЬСКОХОЗЯЙСТВЕННАЯ АТРЕЛЬ", false);
            t.AddVariant("СЕЛЬСКО ХОЗЯЙСТВЕННАЯ АТРЕЛЬ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("АГРОПРОМЫШЛЕННЫЙ КОМПЛЕКС");
            t.AddVariant("АГРОКОМПЛЕКС", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("АГРОПРОМЫШЛЕННЫЙ ПАРК");
            t.AddVariant("АГРОПАРК", false);
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОМБИКОРМОВЫЙ ЗАВОД");
            m_Ontology.Add(t);
            ...
            t.AddAbridge("О.Л.");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ ЛАГЕРЬ") { Acronym = "ООЛ", AcronymCanBeLower = true };
            t.AddAbridge("О/Л");
            t.AddAbridge("О.Л.");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНЫЙ ЛАГЕРЬ") { Acronym = "ДОЛ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ СПОРТИВНЫЙ ЛАГЕРЬ") { Acronym = "ДСЛ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ ЛАГЕРЬ") { Acronym = "ДООЛ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНЫЙ КОМПЛЕКС") { Acronym = "ДОК", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ КОМПЛЕКС") { Acronym = "ДООК", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ ЦЕНТР") { Acronym = "ДООЦ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ ПРОФИЛЬНЫЙ ЦЕНТР") { Acronym = "ДООПЦ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКИЙ ОЗДОРОВИТЕЛЬНО ОБРАЗОВАТЕЛЬНЫЙ СПОРТИВНЫЙ ЦЕНТР") { Acronym = "ДООСЦ", AcronymCanBeLower = true };
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ГИДРОТЕХНИЧЕСКИЙ КОМПЛЕКС") { Tag3 = 1 };
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ОВОЩЕХРАНИЛИЩЕ") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СВИНОКОМПЛЕКС") { Tag3 = 1 };
            t.AddVariant("СВИНОВОДЧЕСКИЙ КОМПЛЕКС", false);
            t.AddVariant("СВИНОКОМПЛЕКС СК", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПОЛЕ") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПОЛЕВОЙ СТАН") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ГУРТ") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("КОШАРА") { Tag3 = 1 };
            m_Ontology.Add(t);
            ...
            t.AddVariant("НАЦИОНАЛЬНЫЙ ПАРК", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ПАРК КУЛЬТУРЫ И ОТДЫХА") { Acronym = "ПКО", Tag3 = 1 };
            t.AddVariant("ПКИО", false);
            t.AddVariant("ПАРК РАЗВЛЕЧЕНИЙ", false);
            t.AddVariant("ПАРК СЕМЕЙНОГО ОТДЫХА", false);
            t.AddVariant("ГОРОДСКОЙ ПАРК КУЛЬТУРЫ И ОТДЫХА", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ЗАИМКА") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("МЫС") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("РЕКА") { Tag3 = 1 };
            t.AddVariant("РЕЧКА", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("БЕРЕГ РЕКИ") { Tag3 = 1 };
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СВАЛКА БЫТОВЫХ ОТХОДОВ") { Tag3 = 1 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("МУЗЕЙ");
            t.AddVariant("МУЗЕЙ УСАДЬБА", false);
            t.AddVariant("МУЗЕЙ ЗАПОВЕДНИК", false);
            m_Ontology.Add(t);
            ...
            t.AddVariant("ВОИН. ЧАСТЬ", false);
            t.AddVariant("ВОЙСК. ЧАСТЬ", false);
            t.AddVariant("ВОЕННАЯ ЧАСТЬ", false);
            t.AddVariant("ВОЕН. ЧАСТЬ", false);
            t.AddVariant("ВОЕНЧАСТЬ", false);
            t.AddAbridge("В/Ч");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ЛОДОЧНАЯ СТАНЦИЯ") { Tag3 = 1 };
            m_Ontology.Add(t);
            ...
            t.AddAbridge("ЖИЛ.К.");
            t.AddAbridge("Ж/К");
            t.AddVariant("ЖИЛОЙ КВАРТАЛ", false);
            t.AddVariant("ЖИЛКВАРТАЛ", false);
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТЕЛЕВЕЩАНИЕ");
            t.AddVariant("ТЕЛЕВИЗИОННОЕ ВЕЩАНИЕ", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ИСПРАВИТЕЛЬНАЯ КОЛОНИЯ") { Tag = Pullenti.Ner.Address.Internal.StreetItemType.Noun, Acronym = "ИК", Tag2 = 0 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ИСПРАВИТЕЛЬНО ТРУДОВАЯ КОЛОНИЯ") { Tag = Pullenti.Ner.Address.Internal.StreetItemType.Noun, Acronym = "ИТК", Tag2 = 0 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ДЕТСКАЯ ТРУДОВАЯ КОЛОНИЯ") { Tag = Pullenti.Ner.Address.Internal.StreetItemType.Noun, Acronym = "ДТК", Tag2 = 0 };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ЛЕЧЕБНО ТРУДОВОЙ ПРОФИЛАКТОРИЙ") { Tag = Pullenti.Ner.Address.Internal.StreetItemType.Noun, Acronym = "ЛТП", Tag2 = 0 };
            m_Ontology.Add(t);
            ...
            t.AddAbridge("П.С.");
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ОЛИМПИЙСКАЯ ДЕРЕВНЯ") { Tag = Pullenti.Ner.Address.Internal.StreetItemType.StdName, Tag3 = 1 };
            t.AddAbridge("ОЛИМП. ДЕРЕВНЯ");
            t.AddAbridge("ОЛИМП. ДЕР.");
            t.AddAbridge("ОЛИМП. Д.");
            m_Ontology.Add(t);
            foreach (string s in new string[] {"АПТЕКА", "АТЕЛЬЕ", "ОТЕЛЬ", "ГОСТИНИЦА", "ХОСТЕЛ", "СТУДИЯ", "ПАРИКМАХЕРСКАЯ", "СТОЛОВАЯ", "КАФЕ", "РЕСТОРАН", "УНИВЕРМАГ", "УНИВЕРСАМ", "СУПЕРМАРКЕТ"}) 
            {
                m_Ontology.Add(new Pullenti.Ner.Core.Termin(s) { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building });
            }
            t = new Pullenti.Ner.Core.Termin("МАГАЗИН") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building };
            t.AddAbridge("МАГ.");
            t.AddAbridge("МАГ-Н");
            m_Ontology.Add(t);
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТОРГОВЫЙ ЦЕНТР") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building, Acronym = "ТЦ", AcronymCanBeLower = true, AcronymCanBeSmart = true };
            t.AddVariant("ТОРГОВИЙ ЦЕНТР", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТОРГОВО ОФИСНЫЙ ЦЕНТР") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building, Acronym = "ТОЦ", AcronymCanBeLower = true, AcronymCanBeSmart = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТОРГОВО ОФИСНЫЙ КОМПЛЕКС") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building, Acronym = "ТОК", AcronymCanBeLower = true, AcronymCanBeSmart = true };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ ЦЕНТР") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building, Acronym = "ТРЦ", AcronymCanBeLower = true, AcronymCanBeSmart = true };
            t.AddVariant("ТОРГОВО РОЗВАЖАЛЬНИЙ ЦЕНТР", false);
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ КОМПЛЕКС") { Tag = Pullenti.Ner.Address.Internal.AddressItemType.Building, Acronym = "ТРК", AcronymCanBeLower = true, AcronymCanBeSmart = true };
