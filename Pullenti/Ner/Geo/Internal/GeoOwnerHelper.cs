    ...
    public static class GeoOwnerHelper
    {
        ...
        internal static bool CanBeHigherToken(Pullenti.Ner.Token rhi, Pullenti.Ner.Token rlo)
        {
            if (rhi == null || rlo == null) 
                return false;
            if (rhi.Morph.Case.IsInstrumental && !rhi.Morph.Case.IsNominative && !rhi.Morph.Case.IsGenitive) 
                return false;
            ...
            if (hi.IsCity && lo.IsRegion) 
            {
                if (hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_TYPE, "город", true) != null || hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_TYPE, "місто", true) != null || hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_TYPE, "city", true) != null) 
                {
                    ...
                    if (s.Contains("городской округ")) 
                    {
                        if (hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "МОСКВА", true) != null || hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "САНКТ-ПЕТЕРБУРГ", true) != null || hi.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "СЕВАСТОПОЛЬ", true) != null) 
                            citiInReg = true;
                    }
                }
            }
            ...
            if (rhi.EndChar <= rlo.BeginChar) 
            {
                if (!rhi.Morph.Class.IsAdjective) 
                {
                    ...
                    {
                        if (MiscLocationHelper.IsUserParamAddress(rhi)) 
                        {
                            if (lo.IsRegion && !lo.IsState) 
                                return true;
                            if (lo.IsCity) 
                                return true;
                        }
        ...
        public static bool CanBeHigher(Pullenti.Ner.Geo.GeoReferent hi, Pullenti.Ner.Geo.GeoReferent lo, Pullenti.Ner.Token rhi = null, Pullenti.Ner.Token rlo = null)
        {
                ...
                if (lo.IsCity) 
                {
                    ...
                    if (hit.Contains("поселение") || hit.Contains("поселок") || hit.Contains("улус")) 
                    {
                        if (lot.Contains("деревня") && hit.Contains("коттеджный поселок")) 
                            return false;
                        if ((((((lot.Contains("село;") || lot.Contains("деревня") || lot.Contains("хутор")) || lot.Contains("аул") || lot.Contains("кишлак")) || lot.Contains("пункт") || lot.Contains("слобода")) || lot.Contains("станица") || lot.Contains("починок")) || lot.Contains("заимка") || lot.Contains("местечко")) || lot.Contains("агрогородок")) 
                            return true;
                        if (lot == "поселение;") 
                        {
            ...
            else if (lo.IsCity) 
            {
                ...
                else 
                {
                    ...
                    if (hit.Contains("сельский округ")) 
                        return false;
                    if (hit.Contains("городской округ")) 
                    {
                        if (lo.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "МОСКВА", true) != null || lo.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "САНКТ-ПЕТЕРБУРГ", true) != null || lo.FindSlot(Pullenti.Ner.Geo.GeoReferent.ATTR_NAME, "СЕВАСТОПОЛЬ", true) != null) 
                            return false;
                    }
            ...
            else if (lo.IsRegion) 
            {
                ...
                if (lot.Contains("аймак")) 
                {
                    if (hit.Contains("район") || hit.Contains("область")) 
                        return true;
                }
