    ...
    public class ExtOntology
    {
        ...
        void _initHash()
        {
            lock (m_Lock) 
            {
                if (m_Hash != null) 
                    return;
                Dictionary<string, Pullenti.Ner.Core.IntOntologyCollection> hash = new Dictionary<string, Pullenti.Ner.Core.IntOntologyCollection>();
                foreach (ExtOntologyItem it in Items) 
                {
                    if (it.Referent != null) 
                ...
                foreach (ExtOntologyItem it in Items) 
                {
                    if (it.Referent != null) 
                    {
                        ...
                        if (!hash.TryGetValue(it.Referent.TypeName, out ont)) 
                            hash.Add(it.Referent.TypeName, (ont = new Pullenti.Ner.Core.IntOntologyCollection() { IsExtOntology = true }));
        ...
        public List<ExtOntologyItem> AttachReferent(Referent r)
        {
            ...
            if (!m_Hash.TryGetValue(r.TypeName, out onto)) 
                return null;
            List<Referent> li;
            li = onto.TryAttachByReferent(r, null, false);
