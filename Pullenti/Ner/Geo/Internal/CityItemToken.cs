...
namespace Pullenti.Ner.Geo.Internal
{
    public class CityItemToken : Pullenti.Ner.MetaToken
    {
        public static List<CityItemToken> TryParseList(Pullenti.Ner.Token t, int maxCount, GeoAnalyzerData ad = null)
        {
            ...
            if (ci == null) 
            {
                ...
                if (((t is Pullenti.Ner.TextToken) && t.IsValue("МУНИЦИПАЛЬНЫЙ", null) && t.Next != null) && t.Next.IsValue("ОБРАЗОВАНИЕ", null)) 
                {
                    ...
                    if (lii != null && lii[0].Typ == ItemType.Noun) 
                    {
                        ...
                        {
                            if (t.Previous.IsValue("ГОРОДСКОЙ", null) || t.Previous.IsValue("СЕЛЬСКИЙ", null)) 
                                lii[0].BeginToken = t.Previous;
                        }
                        return lii;
                    }
                }
            ...
            for (t = ci.EndToken.Next; t != null; t = t.Next) 
            {
                ...
                if (ci0 == null) 
                {
                    ...
                    if ((ci0 == null && ((ci.Typ == ItemType.ProperName || ci.Typ == ItemType.City)) && t.IsComma) && li[0] == ci) 
                    {
                        ...
                        {
                            CityItemToken ci1 = TryParse(t.Next, null, false, null);
                            if (ci1 != null && ci1.Typ == ItemType.Noun && ((ci1.EndToken.Next == null || ci1.EndToken.Next.IsComma))) 
                                ci0 = ci1;
                        }
                    }
                    if (ci0 == null) 
            ...
            if ((li.Count >= 2 && li[0].Typ == ItemType.ProperName && li[1].Typ == ItemType.Noun) && li[1].Doubtful) 
            {
            }
            return li;
        }
        ...
        public bool GeoObjectAfter
        {
            get
            {
                return m_GeoObjectAfter;
            }
            set
            {
                m_GeoObjectAfter = value;
            }
        }
        bool m_GeoObjectAfter;
        ...
        public static CityItemToken TryParse(Pullenti.Ner.Token t, CityItemToken prev = null, bool dontNormalize = false, GeoAnalyzerData ad = null)
        {
            ...
            {
                CityItemToken res0 = new CityItemToken(t, t) { Typ = ItemType.Noun, Value = "населенный пункт", Doubtful = true };
            ...
            if (d != null && d.NoGeo) 
                return null;
            if ((t is Pullenti.Ner.TextToken) && (t as Pullenti.Ner.TextToken).Term.StartsWith("УЧ")) 
            {
                Pullenti.Ner.Core.TerminToken ttt = Pullenti.Ner.Address.Internal.AddressItemToken.m_Plot.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No);
                if (ttt != null) 
                {
                    bool ok = false;
                    ...
                    if (ok) 
                    {
                        CityItemToken next = TryParse(ttt.EndToken.Next, null, false, null);
                        if (next != null && next.Typ == ItemType.ProperName) 
                            return new CityItemToken(t, ttt.EndToken) { Typ = ItemType.Noun, Value = "участок" };
                    }
                }
            }
            ...
            if (((res != null && res.Typ == ItemType.ProperName && res.Value != null) && !res.Doubtful && res.BeginToken == res.EndToken) && res.Value.Length > 4) 
            {
                ...
                {
                    if (res.AltValue != null && res.AltValue.EndsWith("А")) 
                    {
                    }
                    else 
                    {
            ...
            if (res != null && ((res.Typ == ItemType.City || res.Typ == ItemType.ProperName)) && MiscLocationHelper.IsUserParamAddress(res)) 
            {
                ...
                {
                    Pullenti.Ner.NumberToken nt = null;
                    if (tt != null && tt.IsHiphen) 
                        nt = Pullenti.Ner.Core.NumberHelper.TryParseRoman(tt.Next);
                    else if ((res.WhitespacesAfterCount < 3) && tt != null && tt.Chars.IsLatinLetter) 
                        nt = Pullenti.Ner.Core.NumberHelper.TryParseRoman(tt);
                    if (nt != null) 
                    {
                        res.EndToken = nt.EndToken;
                        res.Value = string.Format("{0}-{1}", (res.OntoItem != null ? res.OntoItem.CanonicText : res.Value), nt.Value);
                        res.Typ = ItemType.ProperName;
                        res.OntoItem = null;
                    }
                }
        ...
        static CityItemToken _tryParseInt(Pullenti.Ner.Token t, CityItemToken prev, bool dontNormalize, GeoAnalyzerData ad)
        {
                        ...
                        if (((txt == "Д" || txt == "С" || txt == "C") || txt == "П" || txt == "Х") || ((((txt == "А" || txt == "У")) && MiscLocationHelper.IsUserParamAddress(tt)))) 
                        {
                            if (tt.Chars.IsAllLower || MiscLocationHelper.IsUserParamAddress(tt) || ((tt.Next != null && tt.Next.IsCharOf(".,")))) 
                            {
                                ...
                                if (txt == "А" || txt == "С") 
                                {
                                    ...
                                    if (ait != null) 
                                        return null;
                                    Pullenti.Ner.Address.Internal.StreetItemToken st = Pullenti.Ner.Address.Internal.StreetItemToken.TryParse(tt, null, false, null);
                                    if (st != null && st.Typ == Pullenti.Ner.Address.Internal.StreetItemType.Noun) 
                                        return null;
                                    if (tt.Next != null) 
                                    {
                                        if (tt.Next.IsValue("ЗАКОН", null) || tt.Next.IsValue("ПОСТАНОВЛЕНИЕ", null)) 
                                            return null;
                                    }
                                }
                                if (txt == "У") 
                                ...
                                else if (tt2.Chars.IsAllUpper && (tt2.WhitespacesBeforeCount < 2)) 
                                {
                                    ...
                                    if (tt.Chars.IsAllUpper) 
                                    {
                                        ...
                                        if (rtt != null) 
                                        {
                                            ...
                                            {
                                                List<TerrItemToken> tli = TerrItemToken.TryParseList(ttt2, 3, null);
                                                if (tli != null && tli.Count == 2) 
                                                {
            ...
            if (res.Typ == ItemType.Noun) 
            {
                ...
                {
                    CityItemToken ne = _tryParseInt(res.BeginToken.Next.Next, (res.Value == "ГОРОД" ? res : null), false, ad);
                    if (ne != null && ne.Typ == ItemType.City && ((!ne.Doubtful || !res.Doubtful || MiscLocationHelper.IsUserParamAddress(ne)))) 
                    {
                    }
                    ...
                    else if (ne != null && ne.Typ == ItemType.Noun && ne.EndToken == res.EndToken) 
                    {
                    }
            ...
            if (res.Typ == ItemType.ProperName || res.Typ == ItemType.City || res.CanBeName) 
            {
                ...
                if ((!t1.IsWhitespaceAfter && t1.Next != null && t1.Next.IsHiphen) && !t1.Next.IsWhitespaceAfter) 
                {
                    if ((t1.Next.Next is Pullenti.Ner.NumberToken) && (t1.Next.Next as Pullenti.Ner.NumberToken).IntValue != null && ((t1.Next.Next as Pullenti.Ner.NumberToken).IntValue.Value < 30)) 
                        num = t1.Next.Next as Pullenti.Ner.NumberToken;
                    else if (t1.Next.Next is Pullenti.Ner.TextToken) 
                        num = Pullenti.Ner.Core.NumberHelper.TryParseRoman(t1.Next.Next);
                }
                if (num != null) 
                {
                    res.EndToken = (num.Typ == Pullenti.Ner.NumberSpellingType.Roman ? num.EndToken : (Pullenti.Ner.Token)num);
                    res.Value = string.Format("{0}-{1}", val, num.Value);
                    if (res.AltValue != null) 
                        res.AltValue = string.Format("{0}-{1}", res.AltValue, num.Value);
                    res.Typ = ItemType.ProperName;
                    res.OntoItem = null;
                }
        ...
        static CityItemToken _TryParse(Pullenti.Ner.Token t, CityItemToken prev, bool dontNormalize, GeoAnalyzerData ad)
        {
            ...
            if ((li == null && (t is Pullenti.Ner.TextToken) && !(t as Pullenti.Ner.TextToken).Term.EndsWith("О")) && ((t.Next == null || ((!t.Next.IsHiphen && !t.Next.IsValue("ПОСАД", null)))))) 
            {
                if (t.IsValue("ИМ", null) || t.IsValue("ИМЕНИ", null) || ((t.GetMorphClassInDictionary().IsProperName && MiscLocationHelper.IsUserParamAddress(t) && !t.IsValue("ВЛАДИМИР", null)))) 
                {
                    NameToken nam = NameToken.TryParse(t, GeoTokenType.City, 0, false);
                    if (nam != null && nam.EndToken != t) 
                    {
                        if (t.GetMorphClassInDictionary().IsProperName) 
                        {
                            List<TerrItemToken> ters = TerrItemToken.TryParseList(t.Next, 5, null);
                            if (ters != null) 
                            {
                                Pullenti.Ner.ReferentToken ttt = TerrDefineHelper.TryDefine(ters, ad, false, null, null);
                                if (ttt != null) 
                                    nam = null;
                            }
                        }
            ...
            if (li == null && (t is Pullenti.Ner.TextToken) && t.IsValue("КР", null)) 
            {
                Pullenti.Ner.Token tt2 = t.Next;
                if (tt2 != null && ((tt2.IsChar('.') || tt2.IsHiphen))) 
                    tt2 = tt2.Next;
                CityItemToken next = _tryParseInt(tt2, prev, dontNormalize, ad);
                if (next != null && next.Typ == ItemType.ProperName) 
                {
                    next.BeginToken = t;
                    next.Value = "КРАСНЫЙ " + next.Value;
                    return next;
                }
            }
                ...
                if (t is Pullenti.Ner.TextToken) 
                {
                    ...
                    if (li != null) 
                    {
                        foreach (Pullenti.Ner.Core.IntOntologyToken nt in li) 
                        {
                            if (nt.Item != null) 
                            {
                                if (!Pullenti.Ner.Core.MiscHelper.IsAllCharactersLower(nt.BeginToken, nt.EndToken, false) || ((prev != null && prev.Typ == ItemType.Noun))) 
                                {
                                    CityItemToken ci = new CityItemToken(nt.BeginToken, nt.EndToken) { Typ = ItemType.City, OntoItem = nt.Item, Morph = nt.Morph };
                ...
                if (li != null) 
                {
                    foreach (Pullenti.Ner.Core.IntOntologyToken nt in li) 
                    {
                        if (nt.Item == null) 
                        {
                            ...
                            if (ci.Value == "СТАНЦИЯ" && t.Previous != null) 
                            {
                                if (t.Previous.IsValue("НЕФТЕПЕРЕКАЧИВАЮЩАЯ", null)) 
                                    return null;
                            }
                            ...
                            if (t.IsValue("ДЕР", null)) 
                            {
                                if (t.Previous != null && t.Previous.IsValue("ФОН", null)) 
                                    return null;
                            }
                            if ((prev != null && prev.Typ == ItemType.Noun && (t is Pullenti.Ner.TextToken)) && t.IsValue("СЕЛО", null) && !t.Chars.IsAllLower) 
                            {
                                if (MiscLocationHelper.IsUserParamAddress(t)) 
                                    return new CityItemToken(t, t) { Typ = ItemType.ProperName, Value = (t as Pullenti.Ner.TextToken).Term };
                            }
                            ...
                            if (nt.BeginToken.Kit.BaseLanguage.IsRu) 
                            {
                                if (nt.BeginToken.IsValue("Г", null)) 
                                {
                                    ...
                                    else 
                                    {
                                        ...
                                        {
                                            ok = false;
                                            CityItemToken next = TryParse(nt.EndToken.Next, ci, false, null);
                                            if (next != null && next.Typ == ItemType.City) 
                                            {
                                                if (nt.LengthChar == 1 && !nt.IsWhitespaceBefore && (t.Previous is Pullenti.Ner.NumberToken)) 
                                                    return null;
                                                if (next.BeginToken.GetMorphClassInDictionary().IsProperSurname) 
                                                {
                                                }
                                                else 
                                                {
                                            ...
                                            return null;
                                        else if (t.Previous != null && Pullenti.Ner.Address.Internal.StreetItemToken.CheckKeyword(t.Previous)) 
                                        {
                                            if (Pullenti.Ner.Address.Internal.AddressItemToken.CheckStreetAfter(t.Previous, false)) 
                                                return null;
                                        }
                                        if (!ok) 
                                        {
                            ...
                            if (ci.EndToken.Next != null && ci.EndToken.Next.IsChar('.')) 
                            {
                                if (ci.LengthChar < 3) 
                                    ci.EndToken = ci.EndToken.Next;
                                else if (t.Previous != null && t.Previous.IsComma && MiscLocationHelper.IsUserParamAddress(t)) 
                                    ci.EndToken = ci.EndToken.Next;
                            }
                            return ci;
                        }
            ...
            if ((t as Pullenti.Ner.TextToken).Term == "НЕТ") 
                return null;
            if (t.IsValue("МУНИЦИПАЛЬНЫЙ", null)) 
                return null;
            ...
            if (MiscLocationHelper.CheckTerritory(t) != null) 
                return null;
            ...
            {
                Pullenti.Ner.Core.TerminToken ptok = NameToken.m_PrefOnto.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No);
                if (ptok != null) 
                {
                    Pullenti.Ner.Token tt = ptok.EndToken.Next;
                    if (tt != null && tt.IsHiphen) 
                        tt = tt.Next;
                    CityItemToken next = TryParse(tt, prev, dontNormalize, ad);
                    if (next != null && ((next.Typ == ItemType.City || next.Typ == ItemType.ProperName)) && next.BeginToken == next.EndToken) 
                    {
                        if (next.Value == null) 
                            next.Value = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(next, Pullenti.Ner.Core.GetTextAttr.No);
                        next.Value = string.Format("{0}{1}", ptok.Termin.CanonicText, next.Value);
                        next.OntoItem = null;
                        next.BeginToken = t;
                        return next;
                    }
            ...
            for (Pullenti.Ner.Token tt = t; tt != null; tt = tt.Next) 
            {
                ...
                Pullenti.Ner.Core.IntOntologyToken kw = CheckKeyword(tt.Next.Next);
                if (kw != null) 
                {
                    if (kw.IsNewlineAfter || kw.EndToken.Next == null || kw.EndToken.Next.IsComma) 
                    {
                    }
                    else 
                        break;
            ...
            if (k > 0) 
            {
                ...
                {
                    if (Pullenti.Morph.LanguageHelper.IsCyrillicVowel(name[ii]) || Pullenti.Morph.LanguageHelper.IsLatinVowel(name[ii])) 
                        ww++;
                    else 
                        nw++;
                }
            ...
            if (npt != null) 
            {
                ...
                if (MiscLocationHelper.IsUserParamGarAddress(npt)) 
                {
                }
                else if (mc.IsNoun) 
                {
            ...
            if (npt != null) 
            {
                for (Pullenti.Ner.Token ttt = npt.BeginToken; ttt.EndChar < npt.EndChar; ttt = ttt.Next) 
                {
                ...
                if (!npt.EndToken.Chars.Equals(t.Chars)) 
                {
                    ...
                    else 
                    {
                        ...
                        else 
                        {
                            ...
                            {
                                Pullenti.Ner.Token ttt2 = npt.EndToken.Next;
                                if (ttt2 != null && ttt2.IsComma) 
                                    ttt2 = ttt2.Next;
                                List<TerrItemToken> tli = TerrItemToken.TryParseList(ttt2, 3, null);
                                if (tli != null && tli.Count >= 2 && ((tli[0].TerminItem != null || tli[1].TerminItem != null))) 
                                {
                                }
                                else 
                                    npt = null;
            ...
            if (t.Next != null && t.Next.Chars.Equals(t.Chars) && !t.IsNewlineAfter) 
            {
                ...
                else if (CityItemToken.CheckKeyword(t.Next) != null) 
                {
                }
        ...
        static CityItemToken _tryParseSpec(Pullenti.Ner.Token t)
        {
            ...
            if (tok1 != null) 
            {
                ...
                if ((t.LengthChar < res.Value.Length) && (t is Pullenti.Ner.TextToken) && t == res.EndToken) 
                    res.AltValue = (t as Pullenti.Ner.TextToken).Term;
                return res;
            }
