...
namespace Pullenti.Ner.Core
{
    ...
    public class IntOntologyCollection
    {
        ...
        public TerminCollection m_Termins = new TerminCollection();
        ...
        {
            TerminParseAttr attrs = TerminParseAttr.IgnoreBrackets;
            if (canBeGeoObject) 
                attrs |= TerminParseAttr.CanBeGeoObject;
            List<TerminToken> tts = m_Termins.TryParseAll(t, attrs);
            if (tts == null) 
                return null;
