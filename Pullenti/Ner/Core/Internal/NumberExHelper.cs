    ...
    static class NumberExHelper
    {
        public static Pullenti.Ner.Core.NumberExToken TryParseNumberWithPostfix(Pullenti.Ner.Token t)
        {
                    ...
                    else 
                    {
                        ...
                        if (toks1 != null) 
                        {
                            if (((Pullenti.Ner.Core.NumberExType)toks1.Termin.Tag) == Pullenti.Ner.Core.NumberExType.Money) 
                            {
                                ...
                                {
                                    Pullenti.Ner.Core.NumberExToken res = new Pullenti.Ner.Core.NumberExToken(t, toks1.EndToken.Next, nt.Value, nt.Typ, Pullenti.Ner.Core.NumberExType.Money) { RealValue = f, IsReal = isReal, AltRealValue = altf, Morph = toks1.BeginToken.Morph };
                                    return _correctMoney(res, toks1.BeginToken);
                                }
                            }
                        }
            ...
            if (isDollar != null) 
            {
                ...
                return new Pullenti.Ner.Core.NumberExToken(t0, te, "", nt.Typ, Pullenti.Ner.Core.NumberExType.Money) { RealValue = f, AltRealValue = altf, IsReal = isReal, ExTypParam = isDollar };
            }
            ...
            if (toks == null && t1.IsChar('р')) 
            {
                ...
                for (Pullenti.Ner.Token ttt = t0.Previous; ttt != null && cou > 0; ttt = ttt.Previous,cou--) 
                {
                    ...
                    return new Pullenti.Ner.Core.NumberExToken(t, toks.EndToken, nt.Value, nt.Typ, ty) { RealValue = f, AltRealValue = altf, IsReal = isReal, Morph = toks.BeginToken.Morph, ExTypParam = "RUB" };
                }
            }
            if (toks != null) 
            {
                ...
                Pullenti.Ner.Core.NumberExType ty = (Pullenti.Ner.Core.NumberExType)toks.Termin.Tag;
                Pullenti.Ner.Core.NumberExToken res = new Pullenti.Ner.Core.NumberExToken(t, t1, nt.Value, nt.Typ, ty) { RealValue = f, AltRealValue = altf, IsReal = isReal, Morph = toks.BeginToken.Morph };
            ...
            {
                if (((t1 as Pullenti.Ner.TextToken).Term == "RUR" || (t1 as Pullenti.Ner.TextToken).Term == "USD" || (t1 as Pullenti.Ner.TextToken).Term == "EUR") || (t1 as Pullenti.Ner.TextToken).Term == "CNY") 
                {
                    if (t1.Next == null || t1.IsWhitespaceAfter || t1.IsChar(',')) 
                        return new Pullenti.Ner.Core.NumberExToken(t, t1, nt.Value, nt.Typ, Pullenti.Ner.Core.NumberExType.Money) { RealValue = f, AltRealValue = altf, ExTypParam = (t1 as Pullenti.Ner.TextToken).Term };
                }
            }
