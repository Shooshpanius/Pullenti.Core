...
namespace Pullenti.Ner.Booklink.Internal
{
    public class BookLinkToken : Pullenti.Ner.MetaToken
    {
        ...
        public static BookLinkToken TryParseAuthor(Pullenti.Ner.Token t, Pullenti.Ner.Person.Internal.FioTemplateType prevPersTemplate = Pullenti.Ner.Person.Internal.FioTemplateType.Undefined)
        {
            if (t == null) 
                return null;
            Pullenti.Ner.ReferentToken rtp = Pullenti.Ner.Person.Internal.PersonItemToken.TryParsePerson(t, prevPersTemplate, false);
            if (rtp != null) 
            {
                BookLinkToken re;
                if (rtp.Data == null) 
        ...
        static BookLinkToken _tryParse(Pullenti.Ner.Token t, int lev)
        {
            ...
            if (t is Pullenti.Ner.TextToken) 
            {
                ...
                if ((t.LengthChar == 1 && t.IsNewlineBefore && t.Next != null) && t.Next.IsChar('.') && (t.Next.Next is Pullenti.Ner.NumberToken)) 
                {
                    BookLinkToken res = new BookLinkToken(t, t.Next.Next) { Typ = BookLinkTyp.Number };
                    t = t.Next.Next;
                    res.Value = string.Format("{0}.{1}", term, (t as Pullenti.Ner.NumberToken).Value);
                    if (t.Next != null && t.Next.IsChar('.')) 
                    {
                        res.EndToken = (t = t.Next);
                        if (t.Next != null && !t.IsWhitespaceAfter && (t.Next is Pullenti.Ner.NumberToken)) 
                        {
                            res.EndToken = (t = t.Next);
                            res.Value = string.Format("{0}.{1}", res.Value, (t as Pullenti.Ner.NumberToken).Value);
                            if (t.Next != null && t.Next.IsChar('.')) 
                                res.EndToken = (t = t.Next);
                        }
                    }
        ...
        public static bool CheckLinkBefore(Pullenti.Ner.Token t0, string num)
        {
            ...
            {
                int n0 = _getNum((t0.Previous.GetReferent() as Pullenti.Ner.Booklink.BookLinkRefReferent).Number);
                if (n0 > 0 && (n0 + 1) == n1) 
                    return true;
            }
            return false;
        }
        static int _getNum(string val)
        {
            if (val == null) 
                return 0;
