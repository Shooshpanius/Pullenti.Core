    ...
    public class DateAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        public override Dictionary<string, byte[]> Images
        {
            get
            {
                ...
                res.Add(Pullenti.Ner.Date.Internal.MetaDate.DateFullImageId, Pullenti.Ner.Core.Internal.ResourceHelper2.GetBytes("datefull.png"));
                res.Add(Pullenti.Ner.Date.Internal.MetaDate.DateImageId, Pullenti.Ner.Core.Internal.ResourceHelper2.GetBytes("date.png"));
                res.Add(Pullenti.Ner.Date.Internal.MetaDate.DateRelImageId, Pullenti.Ner.Core.Internal.ResourceHelper2.GetBytes("daterel.png"));
                res.Add(Pullenti.Ner.Date.Internal.MetaDateRange.DateRangeImageId, Pullenti.Ner.Core.Internal.ResourceHelper2.GetBytes("daterange.png"));
                res.Add(Pullenti.Ner.Date.Internal.MetaDateRange.DateRangeRelImageId, Pullenti.Ner.Core.Internal.ResourceHelper2.GetBytes("daterangerel.png"));
                return res;
            }
        }
        ...
        public static List<Pullenti.Ner.ReferentToken> TryAttach(List<Pullenti.Ner.Date.Internal.DateItemToken> dts, bool high)
        {
            if (dts == null || dts.Count == 0) 
                return null;
            if (!dts[0].IsWhitespaceBefore) 
            {
                if ((((dts[0].BeginToken.Previous is Pullenti.Ner.TextToken) && dts[0].BeginToken.Previous.Chars.IsLetter)) || (dts[0].BeginToken.Previous is Pullenti.Ner.NumberToken)) 
                {
                    if (!dts[dts.Count - 1].IsWhitespaceAfter) 
                    {
                        Pullenti.Ner.Token tt2 = dts[dts.Count - 1].EndToken.Next;
            ...
            if (!b) 
            {
                ...
                if ((dts.Count == 1 && dts[0].Typ == Pullenti.Ner.Date.Internal.DateItemToken.DateItemType.Month && dts[0].BeginToken.Previous != null) && ((dts[0].BeginToken.Previous.Morph.Class.IsPreposition || dts[0].BeginToken.Previous.IsCommaAnd))) 
                {
                ...
                if ((dts.Count == 1 && dts[0].Typ == Pullenti.Ner.Date.Internal.DateItemToken.DateItemType.Number && dts[0].CanBeYear) && dts[0].LengthChar == 4) 
                {
                    if (dts[0].IsNewlineBefore && dts[0].EndToken.Next != null && dts[0].EndToken.Next.IsHiphen) 
                    {
                        List<Pullenti.Ner.ReferentToken> res0 = new List<Pullenti.Ner.ReferentToken>();
        ...
        static bool ApplyRuleWithMonth(List<Pullenti.Ner.Date.Internal.DateItemToken> its, bool high, out Pullenti.Ner.Date.Internal.DateItemToken year, out Pullenti.Ner.Date.Internal.DateItemToken mon, out Pullenti.Ner.Date.Internal.DateItemToken day, out bool yearIsDiff)
        {
            ...
            if ((its.Count > 2 && its[0].CanBeDay && its[1].Typ == Pullenti.Ner.Date.Internal.DateItemToken.DateItemType.Month) && !its[2].CanBeYear && its[2].Typ == Pullenti.Ner.Date.Internal.DateItemToken.DateItemType.Number) 
            {
                mon = its[1];
                day = its[0];
                return true;
            ...
            int i1 = i;
            int yearVal = 0;
            if (lang == null || (((lang.IsRu || lang.IsIt || lang.IsBy) || lang.IsUa))) 
            {
            ...
            for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
            {
                ...
                else if (str == "ПО" || str == "ДО" || str == "BEFORE") 
                {
                    if ((t.Next is Pullenti.Ner.ReferentToken) && (t.Next.GetReferent() is DateReferent)) 
                    {
                        ...
                        if ((t.Previous != null && t.Previous.IsHiphen && t.Previous.Previous != null) && (t.Previous.Previous.GetReferent() is DateReferent)) 
                        {
                            dr.DateFrom = t.Previous.Previous.GetReferent() as DateReferent;
                            t00 = t.Previous.Previous;
                        }
                        Pullenti.Ner.ReferentToken rt0 = new Pullenti.Ner.ReferentToken(ad.RegisterReferent(dr), t00, t.Next);
