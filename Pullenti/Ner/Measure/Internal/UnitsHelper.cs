...
namespace Pullenti.Ner.Measure.Internal
{
    public static class UnitsHelper
    {
        ...
        public static void Initialize()
        {
            ...
            t.AddAbridge("T.");
            Termins.Add(t);
            foreach (UnitsFactors f in new UnitsFactors[] {UnitsFactors.Kilo, UnitsFactors.Mega}) 
            {
                _addFactor(f, uu, "Т", "T", "ТОННА;ТОННЫЙ", "ТОННА;ТОННИЙ", "TONNE;TON");
            }
            ...
            Termins.Add(t);
            u = new Unit("рад", "rad", "рад", "rad");
            Units.Add(u);
            u.Keywords.AddRange(new string[] {"ДОЗА", "ИЗЛУЧЕНИЕ", "ОБЛУЧЕНИЕ", "ИЗЛУЧАТЬ"});
            t = new Pullenti.Ner.Core.Termin("РАД") { Tag = u };
            t.AddVariant("RAD", false);
            Termins.Add(t);
            foreach (UnitsFactors f in new UnitsFactors[] {UnitsFactors.Kilo, UnitsFactors.Mega}) 
            {
                _addFactor(f, u, "рад", "rad", "рад", "рад", "rad");
            }
            u = new Unit("рад", "rad", "радиан", "radian");
            Units.Add(u);
            u.Keywords.AddRange(new string[] {"УГОЛ", "ВРАЩЕНИЕ"});
            ...
            uGradus.Keywords.AddRange(new string[] {"ТЕМПЕРАТУРА", "ШИРОТА", "ДОЛГОТА", "АЗИМУТ", "НАКЛОН", "ДИФФЕРЕНТ", "TEMPERATURE", "LATITUDE", "LONGITUDE", "AZIMUTH"});
            Units.Add(uGradus);
            ...
            Termins.Add(t);
            uGradusK = new Unit("°К", "°K", "градус Кельвина", "Kelvin degree") { Kind = Pullenti.Ner.Measure.MeasureKind.Temperature };
            uGradusK.Keywords = uGradusC.Keywords;
            uGradusK.Psevdo.Add(uGradusF);
            Units.Add(uGradusK);
            t = new Pullenti.Ner.Core.Termin("ГРАДУС КЕЛЬВИНА") { Tag = uGradusK };
            t.AddVariant("ГРАДУС ПО КЕЛЬВИНУ", false);
            t.AddVariant("KELVIN DEGREE", false);
            Termins.Add(t);
            ...
            u = new Unit("эрг", "erg", "эрг", "erg");
            u.Keywords.AddRange(new string[] {"РАБОТА", "ЭНЕРГИЯ"});
            Units.Add(u);
            t = new Pullenti.Ner.Core.Termin("ЭРГ") { Tag = u };
            t.AddVariant("ERG", false);
            Termins.Add(t);
            foreach (UnitsFactors f in new UnitsFactors[] {UnitsFactors.Kilo, UnitsFactors.Mega, UnitsFactors.Giga, UnitsFactors.Tera, UnitsFactors.Milli}) 
            {
                _addFactor(f, u, "эрг", "erg", "ЭРГ", "ЭРГ", "ERG");
            }
